...
Component({
	 ...
	 * x:触摸时X轴的值
	 * w:滑块可移动的X轴范围
	 * cssAnimation:CSS动画的初始值
	 * succeedMsg:验证成功提示信息的默认值
	 * pullStatus:是否允许验证成功后继续滑动
	 */
	data: {
		hint: '右滑验证', 
		...
		xAxial: 0, 
		x: 0, 
		w: (wx.getSystemInfoSync().windowWidth * 0.8) - 50, 
	...
	methods: {
		...
		moveFun: function (e) {
			...
			if (this.data.pullStatus) {
				...
				console.log("--------------------------");
				console.log('触摸点：' + e.changedTouches[0].clientX);
				console.log('sysW(获取屏幕宽度):' + this.data.sysW);
				console.log('w（滑块可移动的X轴范围）:' + this.data.w);
				console.log((this.data.sysW * 0.1) + 25);
				console.log('x(触摸时X轴的值):'+ this.data.x);
				console.log("--------------------------");
